...
class SpriteActor extends Object2D {
  ...
  static MOVE_COMPONENT = "MOVE";
  static DRAW_COMPONENT = "DRAW";

  constructor(name = "SpriteActor", tileMap) {
    super(name, PlatformMover2DComponent.create(SpriteActor.MOVE_COMPONENT, tileMap));
    ...
    this.add(SpriteComponent.create(SpriteActor.DRAW_COMPONENT));
  }
  ...
  set tileMap(tileMap) {
    this.allComponents.get(SpriteActor.MOVE_COMPONENT).tileMap = tileMap;
  }

  get tileMap() {
    return this.allComponents.get(SpriteActor.MOVE_COMPONENT).tileMap;
  }
  ...
  getProperties() {
    ...
    props.add("Sprite", [
      function () {
        return this.sprite.spriteResource[resourceName + ":" + this.sprite.name];
      }.bind(this),
      null
    ]);
  ...
  update(time, dt) {
    ...
    if (moveVec.length != 0) {
      ...
      dir.x = moveVec.x;
      dir.y = moveVec.y;
      dir.normalize().mul(this.getComponent("move").tileSize);

      rayInfo = RayInfo.create(testPt, dir);

      TileMap.castRay(this.tileMap, rayInfo, this.renderContext);
      ...
      if (rayInfo.shape) {
        ...
        this.callScriptedEvent("onCollideWorld",
          ...
          [rayInfo.shape, rayInfo.impactPoint, dist.length, RenderEngine.worldTime, RenderEngine.deltaTime]);
        dist.destroy();
      }
    ...
    this.callScriptedEvent("onAfterUpdate", ["worldTime", "delta"], [RenderEngine.worldTime, RenderEngine.deltaTime]);
  }
