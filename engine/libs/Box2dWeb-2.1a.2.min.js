...
*/
var a2j={};
(function(L,I){if(I.flash)if(I.flash.utils){if(!I.flash.utils.Dictionary)I.flash.utils.Dictionary=Object}else I.flash.utils={Dictionary:Object};else I.flash={utils:{Dictionary:Object}};I.Vector2=I.Array;Function.prototype.inherit=function(H){var F=Function.prototype.inherit.empty;F.prototype=H.prototype;this.prototype=new F;this.prototype.constructor=this};Function.prototype.inherit.empty=function(){};I.trace=function(){I.console&&I.console.log instanceof Function&&I.console.log.apply(I.console,arguments)};
...
function(d){if(d===undefined)d=0;if(this.m_root!=null)for(var n=0;n<d;n++){for(var e=this.m_root,q=0;e.IsLeaf()==false;){e=this.m_path>>q&1?e.child2:e.child1;q=q+1&31}++this.m_path;this.RemoveLeaf(e);this.InsertLeaf(e)}};U.prototype.GetFatAABB=function(d){return d.aabb};U.prototype.GetUserData=function(d){return d.userData};U.prototype.Query=function(d,n){if(this.m_root!=null){var e=new Vector2,q=0;for(e[q++]=this.m_root;q>0;){var t=e[--q];if(t.aabb.TestOverlap(n))if(t.IsLeaf()){if(!d(t))break}else{e[q++]=
t.child1;e[q++]=t.child2}}}};U.prototype.RayCast=function(d,n){if(this.m_root!=null){var e=n.p1,q=n.p2,t=N.SubtractVV(e,q);t.Normalize();t=N.CrossFV(1,t);var v=N.AbsV(t),z=n.maxFraction,u=new aa,w=0,E=0;w=e.x+z*(q.x-e.x);E=e.y+z*(q.y-e.y);u.lowerBound.x=Math.min(e.x,w);u.lowerBound.y=Math.min(e.y,E);u.upperBound.x=Math.max(e.x,w);u.upperBound.y=Math.max(e.y,E);var P=new Vector2,Y=0;for(P[Y++]=this.m_root;Y>0;){z=P[--Y];if(z.aabb.TestOverlap(u)!=false){w=z.aabb.GetCenter();E=z.aabb.GetExtents();if(!(Math.abs(t.x*
...
new V};ga.prototype.b2FixtureDef=function(){this.userData=this.shape=null;this.friction=0.2;this.density=this.restitution=0;this.filter.categoryBits=1;this.filter.maskBits=65535;this.filter.groupIndex=0;this.isSensor=false};fa.b2Island=function(){};fa.prototype.b2Island=function(){this.m_bodies=new Vector2;this.m_contacts=new Vector2;this.m_joints=new Vector2};fa.prototype.Initialize=function(a,b,f,m,p,D){if(a===undefined)a=0;if(b===undefined)b=0;if(f===undefined)f=0;var B=0;this.m_bodyCapacity=a;
this.m_contactCapacity=b;this.m_jointCapacity=f;this.m_jointCount=this.m_contactCount=this.m_bodyCount=0;this.m_allocator=m;this.m_listener=p;this.m_contactSolver=D;for(B=this.m_bodies.length;B<a;B++)this.m_bodies[B]=null;for(B=this.m_contacts.length;B<b;B++)this.m_contacts[B]=null;for(B=this.m_joints.length;B<f;B++)this.m_joints[B]=null};fa.prototype.Clear=function(){this.m_jointCount=this.m_contactCount=this.m_bodyCount=0};fa.prototype.Solve=function(a,b,f){var m=0,p=0,D;for(m=0;m<this.m_bodyCount;++m){p=
this.m_bodies[m];if(p.GetType()==j.b2_dynamicBody){p.m_linearVelocity.x+=a.dt*(b.x+p.m_invMass*p.m_force.x);p.m_linearVelocity.y+=a.dt*(b.y+p.m_invMass*p.m_force.y);p.m_angularVelocity+=a.dt*p.m_invI*p.m_torque;p.m_linearVelocity.Multiply(L.Clamp(1-a.dt*p.m_linearDamping,0,1));p.m_angularVelocity*=L.Clamp(1-a.dt*p.m_angularDamping,0,1)}}this.m_contactSolver.Initialize(a,this.m_contacts,this.m_contactCount,this.m_allocator);b=this.m_contactSolver;b.InitVelocityConstraints(a);for(m=0;m<this.m_jointCount;++m){D=
this.m_joints[m];D.InitVelocityConstraints(a)}for(m=0;m<a.velocityIterations;++m){for(p=0;p<this.m_jointCount;++p){D=this.m_joints[p];D.SolveVelocityConstraints(a)}b.SolveVelocityConstraints()}for(m=0;m<this.m_jointCount;++m){D=this.m_joints[m];D.FinalizeVelocityConstraints()}b.FinalizeVelocityConstraints();for(m=0;m<this.m_bodyCount;++m){p=this.m_bodies[m];if(p.GetType()!=j.b2_staticBody){var B=a.dt*p.m_linearVelocity.x,O=a.dt*p.m_linearVelocity.y;if(B*B+O*O>A.b2_maxTranslationSquared){p.m_linearVelocity.Normalize();
